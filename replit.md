...
13. **Cloud Storage**: Dual-environment storage system supporting both Replit Object Storage (production) and local filesystem (development). Unified `StorageProvider` interface with discriminated `StorageObjectDescriptor` union ensures type-safe operations across environments. Image uploads, serving, and ACL policy management work seamlessly in both modes. Local development saves images to `local-storage/private/uploads/` directory.
...
15. **Dual Environment Support**: Application runs on both Replit (port 5000) and local Mac (configurable port via PORT env var). Environment detection (`server/env.ts`) automatically switches between Replit Object Storage and local file storage. Storage factory pattern (`server/storageFactory.ts`) provides singleton instances per environment. All image URLs use consistent `/images/:path` format regardless of storage backend.
