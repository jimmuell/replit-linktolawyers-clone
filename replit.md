...
13. **Cloud Storage**: Production-ready image storage with dual-mode support. On Replit: uses Object Storage with proper ACL policies for blog images. For local development: automatically falls back to local file storage in `public/uploads/` directory. Environment detection via `REPL_ID` or `USE_LOCAL_STORAGE` env var. Includes SimpleImageUploader component with drag-and-drop functionality, multipart upload handling via multer, image serving endpoints, and URL normalization utilities. Port is configurable via `PORT` env var (default: 5000 for Replit, can use 5001 locally).
...
15. **Flow Management System**: Admin interface for importing, editing, and previewing multi-step form flows from markdown files. Supports node types: start, form, yes-no, multiple-choice, text, date, info, completion, success, end, and subflow. Flows are stored in localStorage and displayed as clickable cards. Features include conditional branching visualization, flow structure editing, and interactive preview mode. Reference documentation in `docs/node-reference-guide.md`.
16. **Flow Testing System**: Admin feature for rigorous flow validation against test scripts. Test scripts must be in Markdown (.md) format with "## Path X of Y" headings and tables containing Step | Screen Type | Question/Content | Action | Expected Next columns. Features include: exact question text matching (normalized for case/whitespace), node type matching (yes-no, multiple-choice, form, etc.), node count verification per path, automatic flow matching, real-time validation progress, validation summary with statistics, and downloadable markdown test reports with correction suggestions. Validator in `client/src/lib/flowValidator.ts`. Located at `/admin/test-flows`.
17. **SEO System**: Comprehensive search engine optimization with `robots.txt` (crawl directives allowing public pages, blocking admin/private routes), dynamic `/sitemap.xml` server route (auto-includes all public pages and published blog posts with lastmod dates), and reusable `useSEO` hook (`client/src/hooks/useSEO.ts`) providing per-page meta tags (title, description, Open Graph, Twitter Cards, canonical URLs, and hreflang alternate links for bilingual pages). Applied to all public-facing pages across both English and Spanish.
18. **Ad Campaign Tracking**: Dedicated ad landing pages at `/get-a-quote` (English) and `/es/get-a-quote` (Spanish) that auto-open the quote modal on load and log every visit to the `ad_visits` PostgreSQL table (captures UTM params: source, medium, campaign, content, term; language; timestamp; and didStart flag). When users click "Start" in the modal, a PATCH request updates `didStart=true`. After modal close, users are redirected to the homepage. Admin dashboard includes `AdCampaignLinksCard` with copyable URLs and live stats (total visits, started count, start rate, per-language breakdown). API endpoints: POST/PATCH/GET `/api/ad-visits`.
