...
16. **Flow Testing System**: Admin feature for testing flows with uploaded test scripts. Test scripts must be in Markdown (.md) format with "## Path X of Y" headings and tables containing Step | Screen Type | Question/Content | Action | Expected Next columns. Features include: automatic flow matching, simulated test execution with step-by-step progress, real-time pass/fail results, and downloadable markdown test reports with correction suggestions. Located at `/admin/test-flows`.
